...
public class ServerTest1 {
    public static void main(String[] args) throws IOException, TimeoutException {
        ...
        //绑定
        Binding binding = BindingBuilder.bind(queue).to(exchange).with("routingKey").and(new HashMap<String, Object>() {
            {
        ...
        channel.basicQos(1);
        channel.basicConsume(queue.getName(), true, "1", new DefaultConsumer(channel){
            @Override
            public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                System.out.println(body);
                channel.basicPublish("", properties.getReplyTo(), properties, "server1".getBytes(StandardCharsets.UTF_8));
            }
        });
        channel.basicConsume(queue.getName(), new DeliverCallback() {
            @Override
            public void handle(String s, Delivery delivery) throws IOException {
                System.out.println(delivery.getBody());
                channel.basicPublish("", delivery.getProperties().getReplyTo(), delivery.getProperties(), "server1".getBytes(StandardCharsets.UTF_8));
            }
        }, new CancelCallback() {
            @Override
