...
public class TestController implements SmartInitializingSingleton {
    ...
    @Autowired
    private RedisSpringInvokerSampleServiceFacade redisSpringInvokerSampleServiceFacade;

    @Autowired
    private SpringBootInvokerSampleServiceFacade springBootInvokerSampleServiceFacade;
...
//        return testQuery;
//
        return redisSpringInvokerSampleServiceFacade.testQuery(testVo);
...
//        List<TestVo> testVoList = springInvokerSampleServiceFacade.list(Arrays.asList(testVo), jsonObject, Arrays.asList(testVo).stream().collect(Collectors.toSet()), TestVo.class, objectHashMap, HttpStatus.OK);
        List<TestVo> testVoList = springBootInvokerSampleServiceFacade.list(Arrays.asList(testVo), jsonObject, Arrays.asList(testVo).stream().collect(Collectors.toSet()), TestVo.class, objectHashMap, HttpStatus.OK);
        return testVoList;
    ...
    public void testArrays(){
        ...
        testVo.setUserName("admin");
        testVo.setPassword("123456");
        redisSpringInvokerSampleServiceFacade.testArrays(null, new TestVo[]{testVo}, 1, new int[]{0} );
    }

    @RequestMapping("/testVoid")
    public TestVo testVoid(){
        return redisSpringInvokerSampleServiceFacade.testVoid();
    }

    @RequestMapping("/testNull")
    public Object testNull(){
        redisSpringInvokerSampleServiceFacade.testNull(null);
        return "ok";
    }

    @RequestMapping("/testBytes")
    public Object testBytes(){
        return redisSpringInvokerSampleServiceFacade.testByte(new byte[]{1, 2, 3}, new Byte[]{1, 2, 3});
    }
