...
public class InvokerSecurityConfiguration {
    ...
    public SecurityFilterChain invokerSecurityFilterChain(HttpSecurity httpSecurity, InvokerSecurityCustomizer customizer) throws Exception {
        ...
        ignoreUrlSet.forEach(url -> registry.antMatchers(url).permitAll());
        ...
        if (ignoreUrlSet.isEmpty()) {
            CustomizeHttpSecurity = httpSecurity
                    .requestMatchers().mvcMatchers(InvokerConstant.FIXED_URL).anyRequest().and()
            ...
            CustomizeHttpSecurity = httpSecurity
                    ...
                    .csrf(csrf -> csrf.disable())
                    .authorizeRequests(e -> {
                        e.mvcMatchers(ignoreUrlSet.stream().toArray(String[]::new)).permitAll().anyRequest().authenticated();
                    });
        }
